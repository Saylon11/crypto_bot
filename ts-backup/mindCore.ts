...
// Using MINDReport from mindEngine
import { MINDReport } from '../core/mindEngine';
...
export class MindCore {
  ...
  constructor() {
    ...
    if (fatbearMint) {
      ...
      console.log(`🚫 Blacklisted own token: ${fatbearMint.slice(0, 8)}...`);
    }
  }
  ...
  async scanForOpportunities(): Promise<MindDirective> {
    ...
    try {
      console.log('🔍 M.I.N.D. scanning for gems...');
      ...
      console.log(`📊 Found ${eligibleGems.length} potential gems`);
  ...
  private selectBestOpportunity(
    ...
    console.log(`\n🎯 Best opportunity: ${best.token.symbol || best.token.mint.slice(0, 8)}`);
  ...
  addToBlacklist(tokenAddress: string): void {
    ...
    console.log(`🚫 Added to blacklist: ${tokenAddress.slice(0, 8)}...`);
  }
